...
// undeclared-field-dropped-on-decode reason as everything else on this
// list.
// "20" (CHAOS-4012, 2026-08-23): purely additive, telemetry-only -- see the
// producer's own ReportSchemaVersion doc comment for the full mechanism.
// twoTurnCaseResult gained KindOfferExplicitHintCount/KindOfferDistinctKindCount/
// KindOfferSuppressedByCardinality and the Turn1-prefixed twin of all three,
// distinguishing "genuinely 0 offerable kinds in the pool" from "exactly 1,
// still suppressed by kindOfferMaterial's own cardinality gate" -- the
// "in pool but never offered" gap CHAOS-4012 investigates. No merge
...
// "23" (CHAOS-4012, 2026-08-23, rebased onto "22"/CHAOS-4165 -- this branch
// independently claimed "22" before that landed on main, renumbered here):
// purely additive, telemetry-only -- see the producer's own
// ReportSchemaVersion doc comment for the full mechanism. twoTurnCaseResult
// gained CandidateOfferCount/OfferKind and the Turn1-prefixed twin of both
// (the candidate-list offer axis's own pair on the SAME kind_offer stage
// "20" added fields for), plus ExpectedKindAtOfferBoundary (ExpectedInPool's
// call-boundary-scoped refinement, re-smoke follow-up -- single field,
// turn-1-only). No merge arithmetic changes (Results concatenates
...
type twoTurnCaseResult struct {
	...
	// CanonicalFactsCount (CHAOS-4347, schema v36) mirrors twoTurnCaseResult's
	// identically-named field byte-for-byte -- see the producer's own doc
	// comment. Purely additive passthrough; no merge arithmetic.
	...
	// CHAOS-4012 v20: kind_offer's own block, mirroring twoTurnCaseResult's
	// identically-named fields byte-for-byte (no omitempty, same reasoning
	// as CensusRan/CensusComplete/CensusCount below).
	...
	// CHAOS-4012 v22: the candidate-list axis's own pair on the SAME
	// kind_offer stage above, mirroring twoTurnCaseResult's identically-named
	...
	// CHAOS-4012 v20: turn 1's own kind_offer twin, mirroring
	// twoTurnCaseResult's identically-named fields byte-for-byte.
	...
	// v31) mirror twoTurnCaseResult's identically-named fields byte-for-byte
	// -- see the producer's own doc comment for the offer_kind=window_expand
	// derivation. No omitempty, same reasoning as Turn1OfferComposedUnderWindowGate
	// above: false is a reading ("this case's window gate had nothing to
...
func evaluateGates(r twoTurnReport) error {
	...
	// CHAOS-4314: structural partition check, NOT the measurement bar --
	// every window-gated case must land in exactly one of offered/silent
	// (chris "go" 2026-08-26's own success bar, window_gated_silent -> 0, is
