...
def main():
    ...
    m = json.loads(io.open(args.spec, encoding="utf-8").read())
    ...
    if before == after:
        ...
        print("NOMATCH")
        print("digest did NOT move on %s (%s)" % (m["file"], before), file=sys.stderr)
        return 0
    print("APPLIED")
    print("digest-moved %s -> %s on %s" % (before[:12], after[:12], m["file"]), file=sys.stderr)
    return 0
