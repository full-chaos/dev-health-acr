...
def parse(raw):
    ...
    if first.startswith("{"):
        ...
        for i, line in enumerate(raw.splitlines(), 1):
            ...
            try:
                ...
                _fail("PARSE-ERROR line %d: %s" % (i, exc))
            for key in ("id", "file", "needle"):
                if key not in m:
                    _fail("PARSE-ERROR line %d: missing key %s" % (i, key))
            out.append(
                {
                    "id": m["id"],
                    "file": m["file"],
        ...
        for i, line in enumerate(raw.split("\n"), 1):
            ...
            if len(parts) != 5:
                _fail(
                    "PARSE-ERROR line %d: %d tab-separated fields, want 3, 4 or 5 "
                    "(id, file, needle, replacement, kind)" % (i, len(parts))
                )
            mid, path, needle, repl, kind = parts
            if "\\" in needle or "\\" in repl:
                _fail(
                    "PARSE-ERROR line %d: TSV needle/replacement carry a backslash -- escapes "
                    "are not interpreted; use the JSONL form for multi-line needles" % i
                )
            out.append(
                {
                    "id": mid,
    ...
    for m in out:
        ...
        if m["id"] in seen:
            _fail("PARSE-ERROR: duplicate mutant id %s" % m["id"])
        seen.add(m["id"])
        ...
        for ch in m["id"]:
            if not (ch.isalnum() or ch in "._-"):
                _fail(
                    "PARSE-ERROR %s: ids may use only letters, digits, dot, underscore and "
                    "hyphen -- an id is an artifact name and a matrix key" % m["id"]
                )
        if m["id"].startswith("_"):
            _fail(
                "PARSE-ERROR %s: ids starting with '_' are reserved for the harness's own "
                "arms (_BASELINE, _SENTINEL)" % m["id"]
            )
        if m["kind"] not in VALID_KINDS:
            _fail(
                "PARSE-ERROR %s: kind must be one of %s, got %r -- an unrecognised kind is never "
                "defaulted, because defaulting it either way silently changes which rule the row "
                "is held to" % (m["id"], "/".join(VALID_KINDS), m["kind"])
            )
    return form, out
...
def main():
    ...
    if args.table == "-":
        ...
        raw = io.open(args.table, encoding="utf-8").read()
    ...
    if args.out == "-":
        ...
        io.open(args.out, "w", encoding="utf-8").write(lines)
    if not args.quiet:
        print(
            "normalised %d mutants (%s form), %d replacement row(s)"
            % (
                len(mutants),
                form,
                sum(1 for m in mutants if m["kind"] == "replace"),
            ),
            file=sys.stderr,
        )
