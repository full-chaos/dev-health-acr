...
here rather than re-derived at test time: `harness.post` writes exactly two failure
bodies -- the transport arm (`{error: str(e)}`, status 0) and the undecodable-HTTP-error
arm (`{error: "unparseable body"}`) -- and `harness.validate_live_payload` synthesizes a
`failure` ENVELOPE, which is a separate signal the classifier already reads on its own.
Adding a third writer means adding its key here, and the literal-guard pin makes writing
one anywhere else fail.
"""
