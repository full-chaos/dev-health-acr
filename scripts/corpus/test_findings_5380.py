...
def test_the_pin_runner_fails_when_a_declared_pin_file_is_missing():
    ...
    # r8 (astra) P3: everything above reads its "declared" list out of `run_pins.sh`
    # itself, so a suite genuinely dropped from that for-list -- its file still on disk,
    # never run again -- is invisible to it: the guard only checks the runner is
    # internally self-consistent with whatever list it currently has, never that the
    # list is COMPLETE. The independent source of truth is the `test_*.py` files
    # actually present beside the runner; the two must agree, or a silently shrunk
    # declared list passes forever with nothing naming the drop.
    on_disk = {p.name for p in HERE.glob("test_*.py")}
    assert set(declared) == on_disk, (
        "run_pins.sh's declared list has drifted from the test_*.py files on disk -- "
        f"declared but not on disk: {sorted(set(declared) - on_disk)}; "
        f"on disk but not declared (never run): {sorted(on_disk - set(declared))}")
...
def test_the_committed_shape_space_is_regenerable_and_shows_no_divergence():
    ...
    Its 728 cells are a DIFFERENT enumeration from the equivalence pin's own, and from
    ...
    r8 (astra) P3: the original 364-cell sweep never set `body_undecodable` on any built
    attempt, so `served_2xx_undecodable_body` was unreachable here and a mutation
    neutralizing that classification branch regenerated byte-identically to the
    (unaffected) committed artefact -- caught nothing. The sweep now has a
    body-undecodable axis (728 = 364 x 2); the assertions below require that axis to
    actually REACH the class, not merely double the row count.
    """
    ...
    assert len(rows) == 728, f"the sweep is {len(rows)} cells, not the 728 it reports"
    assert all(r["frozen"] for r in rows), "a cell of the committed sweep is not frozen"
    # VALUE-asserting, not presence-only: the undecodable axis must actually reach the
    # class it was added for, and only on a served (2xx) status -- a mutant collapsing
    # `served_2xx_undecodable_body` to `ok_200` changes what this file regenerates,
    # which the byte-identical check above then catches.
    served_undecodable = [r for r in rows if r["status"] == "2xx" and r["body"] == "undecodable"]
    assert served_undecodable, "no cell crosses the 2xx status band with an undecodable body"
    assert all(r["class"] == "served_2xx_undecodable_body" for r in served_undecodable), \
        f"a served+undecodable cell did not classify served_2xx_undecodable_body: " \
        f"{[r for r in served_undecodable if r['class'] != 'served_2xx_undecodable_body']}"
    non_2xx_undecodable = [r for r in rows if r["status"] != "2xx" and r["body"] == "undecodable"]
