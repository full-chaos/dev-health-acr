...
| Context Fabric fact-read scope | `internal/contextfabric/fact_scope.go` | CHAOS-4099: `FactReadScopeResolver` between resolution and `planFactReads`. Where a project/team subject's facts ARE reachable through a typed chain, `SourcePruned` (which asserts "proven nothing missing") is a false claim -- those pairs get their OWN `unexpanded:<outcome>` vocabulary and a degrading state instead. Eligibility is the closed `factScopeEligibility` table; a row is admitted ONLY with a `Chain` citation naming the edges and their prod producer. DISCLOSURE is wider than ACTIVATION: all 3 named policies ship DISABLED (stage 1) and every other row carries policy `none` and never traverses. `RecordFactScopeExpansion` carries the decision basis. Expect `Coverage.Partial` on project/team questions more often than pre-4099 -- pre-adjudicated, not a regression. See `docs/design/context-fabric-fact-scope.md` |
