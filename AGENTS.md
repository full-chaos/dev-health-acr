...
| Context Fabric fact-read scope | `internal/contextfabric/fact_scope.go` | CHAOS-4099: `FactReadScopeResolver` between resolution and `planFactReads`. A project/team subject whose facts are reachable only through a typed chain gets its OWN `unexpanded:<outcome>` vocabulary and a degrading source state -- never `SourcePruned`, which asserts a proof of absence. All expansion policies ship DISABLED (stage 1); `RecordFactScopeExpansion` carries the decision basis. See `docs/design/context-fabric-fact-scope.md` |
