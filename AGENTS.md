...
| Context Fabric graph lifecycle (build-aside-and-swap) | `internal/contextfabric/lifecycle.go`, `internal/contextfabric/pglifecycle`, `internal/contextfabric/falkorgraph/lifecycle.go` | CHAOS-3898 S2a: per-org epoch pointer + monotonic allocator + `serving`/`building`/`grace` CAS state machine, durable per-epoch retire records, per-epoch checkpoints, `KeyResolver`; migrations `0019`/`0020`; every port is nil-safe/unwired in production this slice (epoch 0 == the legacy key, zero behavior change) |
