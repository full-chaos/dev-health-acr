...
| `ACR_CONTEXT_FABRIC_MODEL_TIMEOUT` | `45s` | Bounds one generation attempt (1s–2m). Production runs `120s`, the legal maximum: this timeout binds the model call before `ACR_REQUEST_TIMEOUT` ever gets a chance to, so a request that would otherwise succeed can still fail early on the inner budget (`deploy/kubernetes/acr/overlays/production/config.yaml`). Raising it changes the worst-case math in the retry-topology section below -- recompute `per_attempt`/`per_leg`/`worst_case` before also touching `ACR_REQUEST_TIMEOUT`; this change does not. |
