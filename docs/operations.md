...
| `ACR_CONTEXT_FABRIC_MODEL_TIMEOUT` | `45s` | Bounds one generation attempt (`Config.validate`, 1s–2m, `internal/contextfabric/modelprovider/config.go`). Applies only when a model provider is actually configured. `modelprovider.Configured` treats ANY nonblank `ACR_CONTEXT_FABRIC_MODEL*` variable -- including this timeout alone -- as opting the process into provider validation, and `Config.validate` then requires `ACR_CONTEXT_FABRIC_MODEL_API_KEY` unless `ACR_CONTEXT_FABRIC_MODEL_BASE_URL` is set. Setting the timeout without also setting the provider identity variables (base URL, API key, or both) makes an otherwise-unconfigured deployment fail closed at startup instead of leaving the investigation endpoint's clean per-request 503. Set this only together with the provider identity variables it depends on, not on its own. Production does not set it (no provider is configured there); if a deployment does set it, raising it changes the worst-case math in the retry-topology section below -- recompute `per_attempt`/`per_leg`/`worst_case` before also touching `ACR_REQUEST_TIMEOUT`; this change does not. |
