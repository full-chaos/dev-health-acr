...
| **ordering seam** — frame validity and the refuse basis decide BEFORE any subject commit | the substituted-subject ticket / the self-grouping probe ticket | In Review | PR **#473** · `e867d93b` · 2026-09-07 | Frame validation had shipped as a SHADOW: it validated, recorded and telemetered, and gated nothing, so a REFUSED frame rode out as `Frame: nil` -- indistinguishable one line later from a frame that was never proposed -- and retrieval ran anyway. The verdict is now decided at interpretation (where both frame validity and the refuse basis are knowable and nothing has retrieved anything), carried unconditionally on the family outcome, and BOUND ABOVE BOTH production resolution call sites. Second half: an OFFER is a commit deferred by one turn, so "a vector hit alone never commits a subject" now holds at the offer too -- a vector-only candidate is never offered, and a pre-committed arrival on vector-only evidence is demoted so it can clarify but not commit. Three behaviour changes: a refused frame terminates above retrieval as a SERVED refusal (never a stage error); an offer pool emptied BY THE EXCLUSION clarifies instead of collapsing to `no_match` (a genuinely empty graph keeps it); phase-1 validation is live rather than shadow. | Rig 3x3 replicates, embeddings ON, two private pairs concurrently, sequential: served_with_data **13/13/13** vs baseline 11/12/12; subject substitutions **0/0/0** vs 2/2/2; vector-committed rows 4/4/4 vs 7/6/6; rows stable across all three **34/36** vs 31/36 -- the seam's own noise floor is SMALLER than the baseline's. All 8 declared refuse rows refuse 3/3 (baseline serves two of them 3/3); the nonexistent-subject control never served 3/3 (baseline substitutes 3/3); the PRE-EXISTING substitution is fixed too, 3/3; positive controls 3/3 in both arms. Zero genuine losses: every row that stopped serving is a declared refuse row or a withdrawn fabrication. Suites passed=2284 skipped=10 failed=0 of ran=2294; both arches cross-compile. Pins red-at-parent by content where the symbols exist, mutation-proved where they do not. | THREE mutations SURVIVED and each changed the CODE, not the test: two clauses of the emptied-pool condition were unpinnable and were REMOVED rather than decorated; nothing asserted the VALUE of the gate on the frame-validation line (a hardcoded `passed` satisfied both existing structural guards); and the first pin for the offers-only call site was VACUOUS -- it passed while the gate was removed, because its fixture never entered the regime that reaches that path. The offers-only call site is the substantive design delta: gating only the decisive path left a second, independent resolution running on refused frames and handing callers offers to answer, which is the clarification loop the seam exists to end. A refusal that holds at one of two call sites is not a refusal. |
| **S7b-2** turn-1 two-operand comparison binding | the two-operand resolution ticket | In Review | this PR. | Turn 1 resolves a two-named-operand comparison instead of committing nothing and asking a single-subject question. Operand slot state is **resolver-local and never serialized** — the published subject resolution is a type alias embedded with a JSON tag in two places, so a field on it would be a wire change; none is added. The existing question frame carries the structure, dispatched before the term bag flattens; the six singleton commit gates stay singleton inside per-operand invocations; the evidence census is unreachable in this cut, which is what implements the no-read hold. Anything short of two resolved named operands **holds the whole comparison** and asks once naming both sides; a scoped operand holds before reading. Follow-up selections bind server-side from the current question, unbound rather than guessed on zero or two matches. **D18** replaces D17 limit (a). | Acceptance: the discriminating arm carries a REAL frame and the engine wiring — the earlier probe passed a nil frame and covers only the flat-pool path. Unchanged controls (canonical-hint, exact-label collision, single-subject, the refused operand transition) are **green at parent and green after**; demanding red there manufactures evidence. Cross-layer agreement is asserted in both directions with each frame's slots independently pinned, so common drift cannot pass as equality. A metamorphic count-only arm scrambles matched terms, labels and receipt order and requires identical outcome rows. 14 deletion mutants plus one required wrong-VALUE replacement swapping count-only admission for candidate-term correlation. | Reachability, not just behaviour, moved: this makes the comparison-readiness predicate reachable on turn 1, where it was previously reached only from turn 2. That predicate is **D19**'s subject, so this stacks on the read-population work's post-D19 tip and never lands ahead of it. Three mutants are predicted to survive the core fixtures (duplicate-receipt dedup within a slot, alias normalization semantics, merged candidate-term bounding) and carry their own extension fixtures. Prevalence is **unmeasured** — the probe establishes a capability limitation, not a rate; real client follow-up sufficiency stays unproven until its fixture runs. |
...
**Addendum at landing (2026-09-06).** Recorded verbatim from the read-population ticket's own landing
comment, so this page and that ticket cannot drift apart:

> D19 addendum: readiness for a comparison is evaluated per operand keyed by the declared subject
> kind, fail-closed on a missing standard; sameness is judged comparison-wide at the current row's
> standard; slot-keyed, no identity binding. Contract addendum: the census exception admits
> `population_truncated` only for a row whose role owns a population (member, group, operand) — it is
> refused on a `single_subject` row and on any role outside the closed vocabulary.
...
| **#470** | resolver telemetry (the decision-visibility ticket) | fold every decision event of one resolution into a single Info line under its own stage token, carrying the outcome counts, the committed ids and the gates and bases the commits were taken on; extract the hosted runtime's tracer construction so the emitted line is asserted through the wiring production actually uses | deployed BEFORE/AFTER on the rig: **0 decision lines on 3/3 shapes** at the production level before, **1 decision summary per request** after, positive control inside the BEFORE capture; battery run 2 valid (baseline+sentinel PASS, 1504 ran vs floor 814) killed 10/13 — all **3 survivors were test-side gaps**, each now pinned red-on-mutant; one earlier run **VOID** (bare `go_test_timeout=1200`, no duration unit); luna r1 in progress | **OPEN** |
