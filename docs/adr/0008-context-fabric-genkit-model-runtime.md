...
- `MaxSynthesisResynthesisAttempts` (CHAOS-5655) — bounds `SynthesizeAnswer`'s own re-sampling of a validator-rejected draft; must be in `[1, 3]` (the ceiling equals the documented default -- see "Bounded re-synthesis on invalid output" below for why); a zero value defaults to `1` (no re-sampling, the pre-CHAOS-5655 behavior) rather than the operator-facing default of `3`, which hosted composition applies explicitly.
...
- **Reserved synthesis deadline (2026-09-13, 4452 vol.1 §10 D5 -- "the reserved synthesis deadline ships in the same slice; that is non-negotiable under any option, or the terminal case is a 504 regardless").** The whole HTTP request shares one caller-supplied deadline; a redraw that starts anyway when too little of it remains cannot finish, and a fallback attempted after that failure sees an already-dead context and is correctly, but consequentially, refused by `fallbackWouldSeeADeadContext` (CHAOS-5577) -- a fallback that a faster rejection would have reached. The loop closes this: from the SECOND draw on (the first always runs, exactly as every pre-CHAOS-5655 caller's one attempt did), it checks `ctx.Deadline()` before drawing again, and stops -- falling through immediately to the fallback leg or the existing fail-closed 422, while the deadline still has room left -- when the remaining time is less than the reserved slice. The reserved slice is `Config.Timeout` (one full attempt-budget) when no fallback is configured, or `2 * Config.Timeout` (this redraw plus one full fallback attempt) when one is -- D5's "terminal case" is whatever runs after the decision, and that is the fallback leg whenever one exists, so stopping one attempt-budget too late would still starve it. A `ctx` carrying no deadline at all (`ok == false`) is never refused: there is no shared budget to protect it from, and that residual (a bare cancel, not a deadline) keeps the pre-fix composition risk exactly as documented in the RISK-NOTES this ticket's PR body carries. The reservation is sized off `Config.Timeout` itself -- the same fixed per-attempt ceiling `withRetry` already enforces on every individual call -- rather than a measurement of how long the previous draw actually took, so a single lucky fast draw can never under-reserve the next one. The decision is observable at Info on the same `synthesize` line: `resynthesis_deadline_remaining_ms`, `resynthesis_deadline_reserved_ms` (the reserved slice ACTUALLY used, so a reader can tell the fallback-aware sizing from the plain one without knowing whether a fallback was configured), and `resynthesis_stopped_for_deadline`, emitted only when the check actually ran.
