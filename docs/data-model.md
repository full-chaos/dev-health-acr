...
| `acr.context_fabric_investigation_results` (`0009`) | Immutable persisted `context_fabric_investigation_result.v1` snapshots (`internal/contextfabric/pginvestigation`), read back for `PriorSubjectReceipts` follow-up binding. `0011` adds indexed, save-time-only reuse-key columns (`question_hash`, `contract_version`, `projection_version`, `model_identity`, `source_watermarks`) alongside the immutable `payload` -- CHAOS-3782 answer reuse | Immutable; no purge path yet |
