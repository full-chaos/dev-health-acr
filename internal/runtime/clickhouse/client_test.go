...
func TestClient_Query_translates_named_bindings_without_interpolation(t *testing.T) {
	...
	bindings := []contextpacket.ClickHouseBinding{{Name: "org_id", Value: "org-1'; DROP TABLE evidence"}}
...
func TestTranslateBindings_encodes_nil_for_ClickHouse_native_nullable_value(t *testing.T) {
	// Given
	bindings := []contextpacket.ClickHouseBinding{{Name: "as_of", Value: nil}}
...
func (c *fakeConnection) Query(ctx context.Context, statement string, parameters map[string]string) (contextpacket.ClickHouseRowScanner, error) {
	c.context = ctx
