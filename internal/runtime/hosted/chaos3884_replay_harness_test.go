...
func TestChaos3884ReplayHarness(t *testing.T) {
	...
	for _, i := range indices {
		...
		baselineRes, _, baselineErr := baselineGraph.ResolveSubjects(callCtx, principal, request, interpreted, contextfabric.ResolvedGraphBinding{})
		outcome.BaselineElapsedMS = time.Since(baselineStart).Milliseconds()
		wiredStart := time.Now()
		wiredRes, _, wiredErr := wiredGraph.ResolveSubjects(callCtx, principal, request, interpreted, contextfabric.ResolvedGraphBinding{})
