...
// CHAOS-3849 round-3 (review finding 2) regression: FalkorDB's uniqueness
// constraint on (org, kind, canonical_id) does not apply to a row whose kind
// or canonical_id is NULL, so such a row is NOT covered by the ORDER BY
...
// lands in skippedMalformed (canonical_id decodes to ""), the arithmetic
...
//
// This fake reproduces that corruption for a cypher WITHOUT the round-3 NULL
// guard (page 1 returns oracleFetchBatchSize-1 well-formed rows plus one
// NULL-canonical-id row to fill a full page and keep pagination going; page
// 2 returns that same NULL-keyed row again as its lone, short-page entry) --
// the last well-formed row is never generated by either page at all. A
// cypher WITH the guard is answered honestly (the NULL-keyed row is simply
// never a candidate, so there is nothing unstable to order). Run against
// oracle.go before the round-3 fix (no guard on either query), this test
// fails: the last row is missing from the corpus. Run against the fix (the
// guard on both queries), it passes.
func TestFetchEmbedderFenceCorpusExcludesNullKeyedRowsFromBothQueries(t *testing.T) {
	...
	nullKeyed := row{"n": &node{Properties: map[string]interface{}{
	...
	guardedQuery := func(cypher string) bool {
		return strings.Contains(cypher, propCanonicalID+" IS NOT NULL")
	}
	...
	fake := &fakeConn{queryFunc: func(ctx context.Context, key, cypher string, params map[string]interface{}, readOnly bool) ([]row, error) {
		...
		// PRE-fix corruption: the NULL-keyed row's position under
		...
		case 0:
			...
			rows = append(rows, nullKeyed) // fills the page to exactly oracleFetchBatchSize, so pagination continues.
			return rows, nil
		case oracleFetchBatchSize:
			return []row{nullKeyed}, nil // short page: ends pagination.
		default:
			return nil, nil
		}
	}}
