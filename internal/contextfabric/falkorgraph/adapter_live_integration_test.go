...
func TestLiveFalkorDBContextFabricLifecycle(t *testing.T) {
	...
	hybridResolution, err := adapter.ResolveSubjects(ctx, principal, hybridRequest, interpreted)
	if err != nil {
	...
	rebootstrapped, err := adapter.ResolveSubjects(ctx, principal, request, interpreted)
	if err != nil {
