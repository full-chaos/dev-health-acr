...
func TestAReceiptWhoseIdentityAnswersBothOperandsStaysUnbound(t *testing.T) {
	...
	assertHeldComparison(t, result, comparisonTermA, comparisonTermA, contextfabric.InvestigationClarificationRequired)
}
...
func TestAQuestionOnlySubjectCarriedInOnAReceiptCannotBeBoundToAnOperand(t *testing.T) {
	...
	// NO_MATCH, NOT CLARIFICATION, AND THE FIXTURE IS WHY. Neither operand
	// term retrieves anything and the one whole-question hit is vector-only,
	// which the offer-pool exclusion withholds -- so this hold publishes an
	// EMPTY candidate list, and a comparison publishes no structure material
	// at all. With every offer channel empty there is nothing for the caller
	// to send back, and CHAOS-5637 rules that turn a truthful no_match rather
	// than a question that cannot be answered. The prompt is still carried on
	// the resolution; only the status and the answer sentence differ.
	if got := len(result.SubjectResolution.Candidates); got != 0 {
		t.Fatalf("the fixture published %d candidate(s) -- this arm's whole point is the empty-pool hold, so a non-empty pool means it is asserting the other terminal's rule", got)
	}
	assertHeldComparison(t, result, comparisonTermA, comparisonTermB, contextfabric.InvestigationNoMatch)
}
