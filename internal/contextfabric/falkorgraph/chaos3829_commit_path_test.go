...
func TestResolveSubjects_CommitPathCarveOutFiresThroughTheRealAdapterWiring(t *testing.T) {
	...
	resolution, _, _, _, err := adapter.ResolveSubjects(context.Background(), storage.Principal{OrgID: "org-1", RepositoryScopes: []string{"*"}}, request, interpreted, contextfabric.ResolvedGraphBinding{}, nil, nil, nil)
	if err != nil {
