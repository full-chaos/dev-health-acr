...
func TestResolveSubjects_CommitPathCarveOutFiresThroughTheRealAdapterWiring(t *testing.T) {
	...
	resolution, _, err := adapter.ResolveSubjects(context.Background(), storage.Principal{OrgID: "org-1", RepositoryScopes: []string{"*"}}, request, interpreted, contextfabric.ResolvedGraphBinding{})
	if err != nil {
...
func TestResolveSubjects_CommitPathCarveOutStaysDisabledForScopedPrincipal(t *testing.T) {
	...
	resolution, _, err := adapter.ResolveSubjects(context.Background(), principal, request, interpreted, contextfabric.ResolvedGraphBinding{})
	if err != nil {
		t.Fatalf("ResolveSubjects: %v", err)
	}
