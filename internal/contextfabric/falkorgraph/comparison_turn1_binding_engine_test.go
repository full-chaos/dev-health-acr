...
// vectorOnlyWithheldPasses reports, for one subject, the trace passes on which
// the offer pool withheld it for being vector-only. A pass number per hit, not
// a bare bool: an operand comparison runs one pass per slot, so "withheld" for
// a pair is a claim about which SIDE withheld it, and a count that could not
// tell two slots apart would read the same whether the exclusion ran once or
// on both operands.
func (c *comparisonDecisionTracer) vectorOnlyWithheldPasses(subject contextfabric.SubjectRef) []int {
	c.mu.Lock()
	defer c.mu.Unlock()
	var passes []int
	for _, event := range c.events {
		if event.Stage != "offer_pool" || event.OfferPoolDisposition != "vector_only_excluded" {
			continue
		}
		if subjectKey(event.Subject) == subjectKey(subject) {
...
// wantStatus is STATED BY EACH CALLER, never derived here from the result.
// A hold whose pool carries candidates carries a redeemable offer with them,
// so the clarification stands and the shared projection delivers the question
// with its completion action. A hold whose pool is EMPTY -- a scoped side held
// before retrieval, or a pair whose sides matched nothing -- has no channel the
// caller can answer on, and CHAOS-5637 refuses that document outright
// (assertAnswerableClarification): the turn is a no_match whose limitation
// still says what happened to the pair. Which of the two a fixture produces is
// a property of the fixture, so the fixture says it; computing it here from
// the very result under test would be an expectation that cannot fail.
func assertHeldComparison(t *testing.T, result contextfabric.InvestigationResult, operandOne, operandTwo string, wantStatus contextfabric.InvestigationStatus) {
	t.Helper()
	...
	if result.Status != wantStatus {
		t.Errorf("status = %q, want %q -- a hold that can be answered must clarify so the projection carries its completion action, and one that cannot must say no_match rather than ask a question nothing can redeem",
...
func TestOneResolvedOperandAndOneMissingOperandHoldsTheWholeComparison(t *testing.T) {
	...
	assertHeldComparison(t, result, comparisonTermA, comparisonTermB, contextfabric.InvestigationClarificationRequired)
}
...
func TestANamedOperandPairedWithAScopedOperandHoldsBeforeAnyRead(t *testing.T) {
	...
	// NO_MATCH, NOT CLARIFICATION. The scoped hold fires on the operand
	...
	assertHeldComparison(t, result, comparisonTermA, comparisonScopedAnchorTerm, contextfabric.InvestigationNoMatch)
...
func TestAWholeQuestionOnlySubjectStillCannotCommitOnItsOwn(t *testing.T) {
	...
	tracer := &comparisonDecisionTracer{}
	result, conn := questionOnlyComparisonDrive(t, tracer)
	...
	// the exclusion's own event is what keeps the arm from passing vacuously
	// for a subject retrieval never saw at all.
	withheld := tracer.vectorOnlyWithheldPasses(comparisonQuestionOnlySubject)
	if len(withheld) == 0 {
		t.Fatalf("%s was never withheld as vector-only (offer_pool exclusions seen: none for it) -- the hit is the thing under test, so nothing below this line measures the decision it claims to",
	...
	if subjectCommitted(result.SubjectResolution, comparisonQuestionOnlySubject) {
		t.Errorf("%s committed on a whole-question hit alone -- a vector-only candidate reaching the lone commit gate means the ceiling, the slot exclusion and the offer-pool exclusion are ALL gone, and a subject answering neither operand is now answering the question",
			subjectKey(comparisonQuestionOnlySubject))
	}
	for _, candidate := range result.SubjectResolution.Candidates {
		if subjectKey(candidate.Subject) == subjectKey(comparisonQuestionOnlySubject) {
			t.Errorf("%s was OFFERED as a choice on a whole-question hit alone (withheld on pass(es) %v, then published anyway) -- redeeming that offer next turn substitutes it for an operand the question named",
				subjectKey(comparisonQuestionOnlySubject), withheld)
		}
	}
}
...
// ---------------------------------------------------------------------------
// THE TRACE THE CERTIFIER READS
// ---------------------------------------------------------------------------

// TestAComparisonsTraceCarriesOneBoundAndOneSlotPerOperand executes the two
// multiplicity properties a comparison is the FIRST caller able to break, on
...
//     EXHAUSTIVE over that one group. Two slots each numbering their own terms
//     from 1 would collide on index and disagree on total.
//   - the per-pass summaries (ranked_cut_summary, corroboration_summary,
//     offer_pool_summary) are each declared ExactlyOnePerPass. Two slots
//     finalizing at the same pass number put two of each in one group.
//
// THE ASSERTIONS ARE THE CERTIFIER'S OWN RULES, RESTATED OVER THE CAPTURED
// EVENTS, because the certifier reads a slog LOG and this harness captures
// TRACE EVENTS -- the same values, one step earlier. What is checked here is
// the property, not the plumbing: exhaustiveness is asserted as a set equality
// against 1..total rather than as a count, so a pass that emitted the same
// index twice cannot satisfy it by arithmetic.
func TestAComparisonsTraceCarriesOneBoundAndOneSlotPerOperand(t *testing.T) {
	t.Parallel()
	...
	// --- THE SEARCH BOUND, request-wide ---
	searches := tracer.stage("search")
	if len(searches) < 2 {
		t.Fatalf("the comparison emitted %d search line(s) -- with one term per operand it must emit one per operand, or this arm is measuring a retrieval that never split", len(searches))
	}
	total := searches[0].Total
	indices := map[int]int{}
	for _, event := range searches {
		if event.Total != total {
			t.Errorf("search lines disagree on total (%d vs %d) -- every line in one request must declare the SAME bound, and a per-slot bound is exactly how they stop doing so",
				total, event.Total)
		}
		indices[event.Index]++
	}
	if total != len(searches) {
		t.Errorf("total = %d but the request emitted %d search line(s) -- the declared bound must equal the observed count, or index 1..total is not exhaustive",
			total, len(searches))
	}
	for want := 1; want <= total; want++ {
		...
		case 1:
		case 0:
			t.Errorf("no search line carries index %d of %d -- a gap means one slot restarted its own numbering instead of continuing the request's", want, total)
		default:
			t.Errorf("index %d of %d appears %d times -- two slots numbering from 1 is exactly this collision", want, total, indices[want])
		}
	}

	// --- ONE FINALIZATION PASS PER OPERAND ---
	// Read off a summary the commit machinery emits unconditionally, so a
	// slot that resolved and a slot that did not are counted alike.
	passes := map[int]int{}
	for _, event := range tracer.stage("ranked_cut") {
		if event.RankedCutSummary {
			passes[event.Pass]++
		}
	}
	if len(passes) != 2 {
		t.Errorf("the comparison finalized under %d distinct trace pass(es) (%v), want one per operand -- two operands sharing a pass number put two ExactlyOnePerPass summaries in one group",
			len(passes), passes)
	}
	for pass, count := range passes {
		if count != 1 {
			t.Errorf("pass %d carries %d ranked-cut summaries, want exactly one -- the multiplicity is per pass, so a repeat inside one pass is the same defect as a shared pass number", pass, count)
		}
	}
}

// stage returns every captured event for one trace stage, in emission order.
func (c *comparisonDecisionTracer) stage(name string) []graphrank.ResolutionTraceEvent {
	c.mu.Lock()
	defer c.mu.Unlock()
