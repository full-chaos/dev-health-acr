...
func TestF6_AnInterpreterAxisFlipStillReusesForAnIdenticalRequest(t *testing.T) {
	...
	require.NoError(t, err)
	require.NoError(t, store.Save(ctx, principal, interpretedHistorical, snapshot, &epoch, currentAxisKey, testReuseRetrievalIdentity, testReusePromptVersions, testReuseVersionAuthorities, 0, "", contextfabric.SemanticStateAbsent(contextfabric.SemanticStateAbsenceTurnEndedBeforeInterpretation)))
