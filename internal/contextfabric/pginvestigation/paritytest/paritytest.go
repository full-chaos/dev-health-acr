	...
	for _, testCase := range Cases() {
		t.Run(testCase.Name, func(t *testing.T) {
			...
			for index, step := range testCase.Save {
				err := store.Save(ctx, step.Principal, step.Result, nil, nil, contextfabric.TimeAxisKeyFor(contextfabric.TimeContext{Axis: contextfabric.TemporalCurrent}), contextfabric.ReuseRetrievalIdentity{})
				if step.WantErr {
					if err == nil {
...
func RunExplicitNullDegradedReasonsSuite(t *testing.T, newStore func(t *testing.T) (contextfabric.InvestigationResultStore, RawSeed)) {
	...
	t.Run("save rejects a replay against a stored explicit null result", func(t *testing.T) {
		...
		err := store.Save(context.Background(), orgA, valid, nil, nil, contextfabric.TimeAxisKeyFor(contextfabric.TimeContext{Axis: contextfabric.TemporalCurrent}), contextfabric.ReuseRetrievalIdentity{})
		if err == nil {
