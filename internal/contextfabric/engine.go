...
type EngineOptions struct {
	...
	// DisableAnchorBindingShadow turns OFF the shadow anchor binding
	// (anchor_binding.go): no binding is computed, persisted or reported.
	// Zero value = the shadow runs. Served output is identical either way.
...
type EngineTelemetry interface {
	...
	// RecordConfirmedNeedLedger (CHAOS-5639) reports the per-need
...
func NewEngine(dependencies EngineDependencies, options EngineOptions) (*Engine, error) {
	...
	return &Engine{
		...
		anchorBindingShadowDisabled:                 options.DisableAnchorBindingShadow,
...
func (e *Engine) Investigate(ctx context.Context, principal storage.Principal, request InvestigationRequest) (served InvestigationResult, servedErr error) {
	...
	// Investigate call, here -- before every gate that can end the turn
	// early, not just before the ones that call ResolveSubjects, so every
	// exit below can capture it forward (captureConfirmedNeedLedgerOnly)
	// rather than only the decisive path. It costs none of the
	// interpreter/graph/fact/synthesis work CHAOS-4040's own run-3
	// acceptance bar forbids at the two window gates immediately below:
	// resolveConfirmedNeedLedger only reads the ONE parent this request
	// names through the store, through the SAME choke points
	// (carryLoadResult's memo, carryOriginSameQuestionVerdict) every other
	...
	// CHAOS-5734: the ledger line is published at the EXIT, declared here,
	// above every return that follows the admission, so no gate below --
	// the two window gates and the structure veto included -- can end the
	// turn with an admitted ledger nobody reported. appliedNeeds stays empty
	...
	// request-side canonicalization -- so the reuse key, the axis-conflict
	// veto, the window carry and every default treat a remembered window
	// exactly as they treat the receipt it stands in for
	// (confirmed_need_window.go).
	ledgerWindow := decideLedgerWindow(confirmedNeedLedger, request, windowCanon)
	windowCanon = windowCanon.withRememberedWindow(ledgerWindow)
	...
	if structureCanon.Veto != structureVetoNone {
		...
		// CHAOS-5734: the same echo for a remembered window applied at
	...
	// before the two window gates -- see that call site's own comment. What
	// depends on structureCanon.Confirmed (not known until just above) is
	// computed here instead. appliedNeeds is the SINGLE authority for "does
	...
	confirmedNeedsForCapture := mergeConfirmedNeedsLedger(remembered, confirmedThisTurn, request)
	...
	if !interpretedTimeBound.Answerable() {
		...
		return e.interpretedTimeBoundResult(ctx, principal, request, interpretedTimeBound, binding, timeBoundDispositions, nil, ancestryRoot(request, receiptsValidated(priorValidatedReceipts)), anchorShadow)
	}
	...
	if continuation.Observed {
		...
		if continuation.refusesTurn(familyOutcome.Gate) {
			...
			return e.continuationRefusalResult(ctx, principal, request, binding, refusalDispositions, nil,
				ancestryRoot(request, receiptsValidated(priorValidatedReceipts), refusedCarrier), anchorShadow)
		}
	}
	...
	// canonicalization outcome and reusable Save key below reads: the window
	// carry hit, or a remembered window applied request-side above.
	...
	carriedStructureEntries := []*contractsv1.ContextFabricConfirmedStructureEntry{
		...
		// and for a remembered window applied request-side.
	...
	if effectiveWindow != nil && effectiveWindow.Provenance == WindowInferredDefault {
		...
		anchorShadow.observeResolution(AnchorBindingEvaluationWindowGated, SubjectResolution{}, nil)
		gatedMaterial, gatedMaterialWindowExpandUnavailable := e.gatedOfferMaterial(ctx, principal, request, graphRequest, interpretation, familyOutcome, binding, structureCanon, kindCarry, appliedNeeds, priorEntries, anchorShadow)
	...
	if familyOutcome.Gate.Refuses() {
		...
		return e.terminalResult(ctx, principal, request, interpretation, familyOutcome, gateResolution, substitutionForTelemetry.Outcome, GraphContext{}, reuseWatermarkSnapshot, reuseEpoch, 0, binding, windowCanon, structureCanon, gateMaterial, effectiveWindow, windowCarried, carriedStructureEntriesUnevaluatedExit, &plan, ancestryRoot(request, receiptsValidated(priorValidatedReceipts), driftRefusedParent), e.captureAcceptedReading(request, continuation, familyOutcome, acceptedShape, &plan, derivedRequirements, confirmedNeedsForUnevaluatedExit).withAnchorShadow(anchorShadow).withCarriedParent(confirmedNeedLedger.Parent))
	}
	...
	if err != nil {
		...
		if errors.Is(err, ErrGraphNotProjected) {
			...
			terminal, terminalErr := e.terminalResult(ctx, principal, request, interpretation, familyOutcome, emptyResolution, substitutionForTelemetry.Outcome, GraphContext{}, reuseWatermarkSnapshot, reuseEpoch, 0, binding, windowCanon, structureCanon, structureMaterial, effectiveWindow, windowCarried, carriedStructureEntriesUnevaluatedExit, &plan, ancestryRoot(request, receiptsValidated(priorValidatedReceipts), driftRefusedParent), e.captureAcceptedReading(request, continuation, familyOutcome, acceptedShape, &plan, derivedRequirements, confirmedNeedsForUnevaluatedExit).withAnchorShadow(anchorShadow).withCarriedParent(confirmedNeedLedger.Parent))
	...
	anchorShadow.observeResolution(AnchorBindingEvaluationResolved, resolution, commitBases)
	...
	if len(subjects) == 0 && !workItemTuple {
		terminal, terminalErr := e.terminalResult(ctx, principal, request, interpretation, familyOutcome, resolution, substitutionForTelemetry.Outcome, graphContext, reuseWatermarkSnapshot, reuseEpoch, *subjectCandidatesAuthzDropped, binding, windowCanon, structureCanon, structureMaterial, effectiveWindow, windowCarried, carriedStructureEntriesForServed, &plan, ancestryRoot(request, receiptsValidated(priorValidatedReceipts), driftRefusedParent), e.captureAcceptedReading(request, continuation, familyOutcome, acceptedShape, &plan, derivedRequirements, postVetoLedgerBase).withAnchorShadow(anchorShadow).withCarriedParent(confirmedNeedLedger.Parent))
		return terminal, terminalErr
	}
	...
	if graphContext.Cohort != nil && !workItemTuple {
		...
		seenRankingKind := make(map[FactKind]struct{}, maxRankingFactKinds)
	...
	if graphContext.Cohort != nil {
		...
		if planGroupAxisCollapsed(plan.GroupKind, plan.MemberKind) && !repairAccountedForCollapse {
			...
			terminal, terminalErr := e.terminalResult(ctx, principal, request, interpretation, familyOutcome, collapsedResolution, substitutionForTelemetry.Outcome, GraphContext{}, reuseWatermarkSnapshot, reuseEpoch, 0, binding, windowCanon, structureCanon, structureMaterial, effectiveWindow, windowCarried, carriedStructureEntriesForServed, &plan, ancestryRoot(request, receiptsValidated(priorValidatedReceipts), driftRefusedParent), e.captureAcceptedReading(request, continuation, familyOutcome, acceptedShape, &plan, derivedRequirements, postVetoLedgerBase).withAnchorShadow(anchorShadow).withCarriedParent(confirmedNeedLedger.Parent))
			return terminal, terminalErr
		}
	}
	...
	anchorShadow.observeServedCount(cardinality.Scope)
	if e.telemetry != nil {
