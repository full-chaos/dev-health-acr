...
func (e *Engine) captureAcceptedReading(
	...
	// set, carried into the persisted reading (SemanticScopeAnchor.
	// MemberSource) so a later reuse candidate's OWN stored shape can be
	// checked against it -- see reusedCountIsRepositoryAnchoredTeamCount's
	// call site for why this interim signal exists (no schema change: it
	// rides the existing JSON-encoded semantic-state column). Every exit
	// that has no live GraphContext of its own (every terminal/refusal path)
	// passes CohortMemberSourceNotApplicable, matching what a reuse read of
	// that row will see.
...
func (e *Engine) Investigate(ctx context.Context, principal storage.Principal, request InvestigationRequest) (served InvestigationResult, servedErr error) {
	...
	if windowCanon.Veto != windowVetoNone {
		...
		// here).
		return e.windowVetoResult(ctx, principal, request, windowCanon.Veto, nil, windowCanon.StaleEntry, binding, nil, e.preInterpretExplicitStructure(request), nil, ancestryRoot(request, receiptsNotYetValidated(), vetoingWindowReceiptID(request, windowCanon.Veto)), e.captureConfirmedNeedLedgerOnly(request, remembered, nil))
	}
	...
	if windowCanon.ExplicitUnconfirmed {
		...
		return e.windowConfirmationRequiredResult(ctx, principal, request, nil, *windowCanon.Effective, nil, WindowCanonicalizationGatedExplicitUnconfirmed, binding, StructureOfferMaterial{}, false, nil, nil, nil, ancestryRoot(request, receiptsNotYetValidated()), e.captureConfirmedNeedLedgerOnly(request, remembered, nil))
	}
