...
func (e *Engine) tryReuse(ctx context.Context, principal storage.Principal, request InvestigationRequest, effectiveTimeContext TimeContext, windowKey string, windowKeyEnc windowKeyEncoding, binding ResolvedGraphBinding) (InvestigationResult, bool, bool, error) {
	result, hit, workItemTuple, _, err := e.tryReuseWithReading(ctx, principal, request, effectiveTimeContext, windowKey, windowKeyEnc, binding)
...
func (e *Engine) tryReuseWithReading(ctx context.Context, principal storage.Principal, request InvestigationRequest, effectiveTimeContext TimeContext, windowKey string, windowKeyEnc windowKeyEncoding, binding ResolvedGraphBinding) (InvestigationResult, bool, bool, storedCountReading, error) {
	...
	key := ReuseKey{
		...
		// An EIGHTEENTH conjunctive dimension, same mirrored discipline --
		// see ReuseKey.OwnershipRoutingVersion's own field doc comment for
		// why the routing gate's own arm-selection rules must fence reuse.
		OwnershipRoutingVersion:  e.reuseVersionAuthorities.OwnershipRoutingVersion,
	...
	// A repository-anchored team count is fenced on ReuseKey.
	// OwnershipRoutingVersion (migration 0039) at the SQL layer, before
	// this candidate is even read back -- a row saved under different
