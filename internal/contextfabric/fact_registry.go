...
type FactCapability struct {
	...
	// Obligations declares which AnswerObligations this capability's facts
	// can SERVE, PER SUBJECT KIND. It sits beside Dimension and Tables
	// because it is the same kind of statement: a property of the
	// producer, declared by the producer, never inferred from outside.
	//
	// WHY A DECLARATION RATHER THAN A HAND-WRITTEN SEED. The frozen design
	// carried an obligation -> FactKind seed (state -> status,
	// count -> landscape, ...) chosen from obligation NAMES. Executed
	// against this registry it derived 22 empty requirement cells over the
	// four acceptance questions and the composed cases -- `state` empty on
	// all thirteen frames, because `status` is served for work_item only
	// (workitems.go). The seed was written by reasoning about what an
	// obligation SOUNDS like; only the producer knows what its facts
	// actually establish. So the direction is inverted here: producers
	// declare, and the seed is GENERATED by inverting the declarations
	// (GenerateObligationSeed). A hand-written seed cannot be kept true as
	// producers change; a generated one is wrong only where a declaration
	// is wrong, and a declaration is reviewed next to the query backing it.
	//
	// WHY PER SUBJECT KIND, AND THIS WAS MEASURED RATHER THAN CHOSEN. The
	// first implementation of this field was a flat []AnswerObligation,
	// which is what the design's own sketch named. Run against the
	// CHAOS-4347 parity property it diverged in exactly three cells, in
	// one direction (always an extra kind, never a missing one):
	//
	//	state@team        extra: metrics
	//	state@project     extra: metrics
	//	state@repository  extra: flow
	//
	// Both producers legitimately serve `state` for SOME of their subject
	// kinds and not others -- metrics.go's daily series is a repository
	// state reading but a team rollup it never emits a table for, and
	// flow.go is the reverse. A flat list cannot express that, and no
	// global shape rule recovers it either: requiring a declared table for
	// `state` would still admit metrics@project, which the composition
	// excludes. The obligation a producer serves is a joint property of
	// the producer AND the subject kind, so the declaration is keyed the
	// same way Tables already is.
	//
	// A subject kind with no entry -- or an empty list -- means this
	// capability answers for that subject kind without serving any answer
	// obligation directly. That is true of the substrate producers, whose
	// facts make other producers' facts believable rather than answering
	// an obligation of their own. It is a declaration, not an omission:
	// devhealthfacts' own registry test requires every registered kind to
...
func (c FactCapability) Validate() error {
	...
	if len(c.Obligations) > 32 {
		return fmt.Errorf("fact capability %q declares too many obligation entries", c.Kind)
	}
	for subjectKind, obligations := range c.Obligations {
		// Same rule Tables already enforces: a capability may not declare
		// anything for a subject kind it does not serve. Without it a
		// declaration can silently describe a subject kind the producer
		// was never asked about, and the generated seed would carry a cell
		// no read can ever fill.
		if !supported[subjectKind] {
			return fmt.Errorf("fact capability %q declares obligations for unsupported subject kind %q", c.Kind, subjectKind)
		}
		if len(obligations) > AnswerObligationCount {
			return fmt.Errorf("fact capability %q declares more obligations for subject kind %q than the vocabulary has members", c.Kind, subjectKind)
		}
		seenObligations := make(map[AnswerObligation]bool, len(obligations))
		for _, obligation := range obligations {
			if !ValidAnswerObligation(obligation) {
				return fmt.Errorf("fact capability %q declares an obligation outside the closed vocabulary for subject kind %q", c.Kind, subjectKind)
			}
			if seenObligations[obligation] {
				return fmt.Errorf("fact capability %q declares obligation %q more than once for subject kind %q", c.Kind, obligation, subjectKind)
			}
			seenObligations[obligation] = true
			// A producer may only declare obligations it can serve BY
			// BEING READ. `ranking` and `count` are COMPUTED (§13.2.3): a
			// named server step over already-read facts or over the
			// resolved set, with no FactKinds of their own. Letting a
			// provider declare one would recreate the exact mis-typing
			// that made BAR Q2's `ranking` derive the empty set -- the
			// frozen rule modelled both as reads needing a table shape no
			// producer declares. `evidence` and `coverage` are
			// answer-contract obligations and read nothing.
