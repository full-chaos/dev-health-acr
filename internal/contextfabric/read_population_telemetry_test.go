...
func TestTheTelemetryUnitsAndCensusCannotLie(t *testing.T) {
	...
	t.Run("count_units names what the two integers count", func(t *testing.T) {
		...
		result.Completeness.Outcomes = appendReadRequirementEvaluations(nil, published, coverage,
			readPopulationEvidenceFrom(frame, result, plan, facts, 0, nil))
		stamped := plan
		result.AnswerPlan = &stamped

		events := readRequirementPopulationEventsFrom(frame, result, plan, facts, QuestionFamilyScopedCohortStatus, 0)
		if len(events) == 0 {
	...
	t.Run("population_census comes from the authority, not the row's cause", func(t *testing.T) {
		...
		plan := AnswerPlan{Requirements: published}
		result.Completeness.Outcomes = appendReadRequirementEvaluations(nil, published, coverage,
			readPopulationEvidenceFrom(nil, result, plan, facts, 0, nil))
		stamped := plan
		result.AnswerPlan = &stamped
		...
		events := readRequirementPopulationEventsFrom(nil, result, plan, facts, QuestionFamilyScopedCohortStatus, 0)
		if len(events) != 1 {
