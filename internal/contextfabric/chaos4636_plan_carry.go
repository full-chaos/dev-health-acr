...
const (
	...
	// carrier -- an unreadable context, a graph epoch that moved, or a context
	// stamped by a family definition version not in force. The family-only
	// carry may not serve that same carrier's reading by another route.
	//
	...
	// exists precisely so they never share a label. Folding this into
	// miss_question_drift would put a false basis in the telemetry.
