...
func (e *Engine) finalizeResult(
	...
	// rule is "every input a population needs is already in this scope, so a
	// second carrier would add a way for the two to disagree without adding
	// information." This one ADDS information that is in no other argument:
	// the clamp is applied to MaxCohortMembers BEFORE retrieval runs, so the
