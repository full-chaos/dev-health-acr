...
func TestStore_getDefensiveCopyDoesNotLeakStoredState(t *testing.T) {
	...
	if err := store.Save(ctx, principal, original, nil, nil, contextfabric.TimeAxisKeyFor(contextfabric.TimeContext{Axis: contextfabric.TemporalCurrent})); err != nil {
		t.Fatalf("save: %v", err)
	}
...
func TestStore_saveRespectsContextCancellation(t *testing.T) {
	...
	cancel()
	store := memoryinvestigation.NewStore()
	err := store.Save(ctx, storage.Principal{OrgID: "org-1"}, contextfabric.InvestigationResult{ResultID: "result-cancelled"}, nil, nil, contextfabric.TimeAxisKeyFor(contextfabric.TimeContext{Axis: contextfabric.TemporalCurrent}))
