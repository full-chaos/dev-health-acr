...
func CheckFrameRepairBound(proposed, repaired QuestionFrame, failure FrameValidationFailure) FrameRepairBoundViolation {
	...
	namesKind := reads[FrameFieldSubjectExpressionKind] || reads[FrameFieldSubjectExpressionVariant]

	if repaired.SubjectExpression.Kind != proposed.SubjectExpression.Kind && !namesKind {
	...
	// Emphasis and Dimensions may never be narrowed by a repair. R9's
	// scenario is exactly this: a sampler emits Goals={assess_state} with
	// Emphasis=[negative,positive], I14 fails because no ranking
	// obligation derives, and under a drop-only repair the options were
	// "silently discard Emphasis" (narrowing the answer the 12:42 ruling
	// says must be given) or "refuse" -- with WHICH one happening
	// depending on the sampler's goal pick, reintroducing exactly the
	// instability this design exists to remove. Because Goals is a SET,
	// the repair instead ADDS rank_or_survey, a monotone widening under
	// law L1 that satisfies I14 without discarding anything the user
	// asked for.
	...
	// Temporal may move only when the failed invariant reads it -- I8 is
