...
// producer declares.
//
// WHAT IT STILL CANNOT SEE, stated because the artifact's header would
// otherwise imply completeness. The collector proves what the provider
// source LITERALLY SPELLS: a map literal's keys and string-keyed writes to
// the local holding it. A field emitted through a helper, through a loop
// over a slice of names, or into a map built in another function is
// invisible to it. Round 3 found the first version of exactly this gap --
// it read literals only, and fourteen of the twenty-one providers write
// fields after initialisation -- so the class is known to be live here
// rather than hypothetical. Closing it needs the provider package
// RUNNING, which means real ClickHouse, which the bounded host-only
// command excludes by design; it is carried as a bar for the derivation
// slice.
//
...
// liveCapabilities indexes the real registry by fact kind. It moved here
// from the requirement-trace file when that file was removed: this is now
// its only consumer.
func liveCapabilities(t *testing.T) map[contextfabric.FactKind]contextfabric.FactCapability {
	...
	if len(capabilities) == 0 {
		t.Fatal("the registry is empty; every assertion built on it would be vacuous")
	}
	return capabilities
}
