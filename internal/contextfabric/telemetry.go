...
//
// applied_anchor_basis and anchor_agreement are this axis's own PRE-DECISION/
// POST-DECISION pair for the subject_anchor member specifically: basis names
// how the applied entry came to exist ("" receipt-redeemed, "engine_committed"
...
func (t SlogEngineTelemetry) RecordConfirmedNeedLedger(ctx context.Context, principal storage.Principal, event ConfirmedNeedLedgerEvent) {
	args := append([]any{
		...
		"substitution_parent_result_kind", SanitizeLogAttr(string(parentResultKindToken(event.SubstitutionParentResultKind))),
		"substitution_parent_chain", SanitizeLogAttr(string(parentChainToken(event.SubstitutionParentChain))),
		"substitution_parent_chain_depth", event.SubstitutionParentChainDepth,
		"substitution_parent_chain_error", SanitizeLogAttr(string(noneChainError(event.SubstitutionParentChainError))),
	}, requestIDLogAttrs(ctx)...)
...
func (t SlogEngineTelemetry) RecordFactScopeExpansion(ctx context.Context, principal storage.Principal, event FactScopeExpansionEvent) {
	args := append([]any{
		...
		"organization_grant_authorized_count", event.OrganizationGrantAuthorizedCount,
		"direct_repo_authorized_count", event.DirectRepositoryAuthorizedCount,
		"project_ownership_authorized_count", event.ProjectOwnershipAuthorizedCount,
		"pr_link_authorized_count", event.PullRequestLinkAuthorizedCount,
		"project_ownership_admitted_count", event.ProjectOwnershipAdmittedCount,
		"pr_link_admitted_count", event.PullRequestLinkAdmittedCount,
		"excluded_explicit_text_link_count", event.ExcludedExplicitTextLinkCount,
		"excluded_heuristic_link_count", event.ExcludedHeuristicLinkCount,
		"authorization_grant_measured", event.AuthorizationGrantMeasured,
		"authorization_grant_organization_wide", event.AuthorizationGrantOrganizationWide,
		"authorization_grant_exact_selectors", event.AuthorizationGrantExactSelectors,
		"authorization_grant_owner_selectors", event.AuthorizationGrantOwnerSelectors,
