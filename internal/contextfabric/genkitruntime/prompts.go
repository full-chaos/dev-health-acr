...
// contextFabricCohortSuperlativeTermsList renders contextfabric's own
// CohortSuperlativeJudgmentTerms -- the SAME closed term list
// requireNoSuperlativeClaimOverUnrankableMember enforces structurally -- so
// the words this prompt tells the model never to use about an unrankable
// member and the words the guard actually refuses can never drift apart:
// there is exactly one list, and this only joins it for display.
var contextFabricCohortSuperlativeTermsList = strings.Join(contextfabric.CohortSuperlativeJudgmentTerms, ", ")
...
When the input carries answer_budget, it is the item allowance this answer was PLANNED against, and you should write to fit it rather than write past it and be narrowed afterwards. answer_budget.global is how many items may belong to the answer as a whole; answer_budget.per_member is how many may be about the cohort members; answer_budget.items_per_group is how many may be about each group, repeated across answer_budget.groups groups. An item here means one driver, one finding (in remaining_work, readiness_gaps or conflicts), or one claimed fact; relationship paths are never charged, and the cohort member rows themselves are charged before any of these allowances is published, so per_member is what is LEFT for the drivers, findings and claims you write about members. An item naming several groups costs one item overall and counts against each group it names. These are not additional hard limits on top of the counts above and exceeding them does not invalidate an answer; they are what the answer was budgeted for, and an answer written to them survives intact where a longer one is reduced. Prefer fewer, better-grounded drivers over more of them: a driver dropped later takes its claims with it.
...
A cohort carries two server-computed fields you must read and never contradict: score_meaning and judgment_mismatch. score_meaning is the closed-vocabulary statement of what that cohort's member score/attention_rank/ranking_basis/drivers actually measure -- read it and use ONLY that stated meaning in your own prose about a member's score or rank. Never call a cohort ranking a performance, productivity, capability, or quality judgment, and never use words like best/worst/strongest/weakest/top/bottom performer, UNLESS score_meaning itself names a performance measure (it does not today: the only defined value, "attention", states operational-deficiency, readiness-gap, workload-pressure, health-risk, and investment-mix-concentration pressure, never performance). judgment_mismatch is true when the question's own requested judgment asks for a comparison score_meaning cannot support: when it is true, do not attempt that comparison at all -- state, in your own words, what the score actually measures (paraphrase score_meaning and the cited member's ranking_basis, never just repeat the field name) and say plainly that the requested comparison is not supported by the available data. Regardless of judgment_mismatch, NEVER name a cohort member whose outcome is insufficient_evidence or not_applicable in any superlative or comparative ranking claim -- any of these words (%s), "most"/"least" naming that member ("the most pressured team", never a bare quantifier like "most of its incidents"), any other "-est"/"-iest" superlative form of an adjective (e.g. "riskiest", "unhealthiest"), a phrase like "more <X> than any/all/every other", or a rank number like "#1"/"number one" -- in a driver, a finding, or in direct_judgment/current_state/deterministic_answer -- state that member's outcome instead and leave it out of the ranking claim entirely.
...
Return only the requested structured output.`,
	...
	contextFabricCohortSuperlativeTermsList,
