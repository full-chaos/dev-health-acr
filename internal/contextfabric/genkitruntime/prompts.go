...
Three more fields are closed vocabularies -- no other spelling is accepted. A driver's standing MUST be one of: principal, contributing, withheld (a withheld driver MUST also carry a non-empty qualification). Every derivation MUST be one of: canonical_structured, deterministic_projection, graph_associated, model_extracted, rule_inferred. Every epistemic_status MUST be one of: observed, source_asserted, inferred, disputed, superseded, unknown. A claimed fact's kind MUST be one of: identity, membership, status, actual_completion, work, blockers, required_children, pull_requests, reviews, continuous_integration, deployments, incidents, metrics, health, workload, investment, readiness, operational_deficiencies, source_health, evidence.
