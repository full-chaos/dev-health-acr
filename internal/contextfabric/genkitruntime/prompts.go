...
Fill only the fields that kind uses, and fill them ALL: named_subject uses terms; explicit_set uses operands (each operand is itself a named_subject with terms, or a children_of_scope with anchor_terms and member_kind); discovered_kind uses member_kind; children_of_scope uses anchor_terms and member_kind; grouped_members uses group_kind AND member_kind, set to EXACTLY the kinds the question names -- including when the question groups a kind by that same kind ("each team's status, grouped by team" is grouped_members with group_kind "team" and member_kind "team"). Never re-express a grouped question as discovered_kind or any other variant, and never drop or change group_kind or member_kind to make the grouping look acceptable: whether a grouping is legal is decided by the server after you answer, never by you, and a grouping you rewrite is a question the server can no longer judge; organization_scope uses member_kind only when the question is a count ("how many repositories are in the organization"). terms and anchor_terms follow the same VERBATIM rule as subject_terms. member_kind and group_kind come from the same closed subject-kind set as group_kind above.
