...
Restate the supplied canonical_facts that bear on the question as claimed_facts even when no driver or finding cites them: a claimed fact is a useful answer on its own, so return one for each relevant supplied fact rather than none. Each one must still restate its field and value verbatim and be about the subject that fact names.
