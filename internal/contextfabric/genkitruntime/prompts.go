...
status is a closed vocabulary and one of its values is NOT yours to return: never return status "clarification_required". Clarification is decided by ACR before synthesis runs -- ACR owns the prompt and the options a caller would answer, you have none to offer, and a draft that asks for clarification is discarded rather than passed on. When the supplied data does not support a conclusion, return status "no_match" (which means "nothing here to answer with", not "no subject was found") or "degraded"/"partial" if it supports a weaker one, and say what was missing in limitations.
