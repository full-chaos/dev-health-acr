...
func TestDecisionEventNeverCarriesCorpusText(t *testing.T) {
	...
	interpretFields := map[string]bool{
		...
		// CHAOS-5380 PR-A B4 (r5 P1-1): the PRIMARY leg's own identity,
		// separate from model_id/model_version above (which stay the
		// ANSWER identity). Closed config/version identifiers, never
		// content -- same provenance as model_id/model_version/receipt.Provider.
	...
	synthesizeFields := map[string]bool{
		...
		"fallback_attempts_total": true,
		// CHAOS-5380 PR-A B4 (r5 P1-1): the PRIMARY leg's own identity --
		// see interpretFields' matching entry above for the rationale.
		"primary_provider":      true,
		"primary_model_id":      true,
		"primary_model_version": true,
	}
