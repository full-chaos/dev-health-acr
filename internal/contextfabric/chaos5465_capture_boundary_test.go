...
func TestCaptureBoundary_TheStoredReadingIsTheServedTurnsOwn(t *testing.T) {
	...
	t.Run("the builder reports the plan and gate it is given", func(t *testing.T) {
		...
		withAxis := engine.captureAcceptedReading(request, windowContinuationDecision{}, grouped, ShapeOpen, planWithAxis, derived, CohortMemberSourceNotApplicable, nil)
		cleared := engine.captureAcceptedReading(request, windowContinuationDecision{}, grouped, ShapeOpen, planCleared, derived, CohortMemberSourceNotApplicable, nil)
		...
		refusedCapture := engine.captureAcceptedReading(request, windowContinuationDecision{}, refused, ShapeOpen, planWithAxis, derived, CohortMemberSourceNotApplicable, nil)
