...
func (s *mapResultStore) Save(_ context.Context, principal storage.Principal, result InvestigationResult, _ SourceWatermarkSnapshot, _ RebuildEpoch, _ string, _ ReuseRetrievalIdentity, _ ReusePromptVersions, _ ReuseVersionAuthorities, _ int64, _ string, semantic SemanticStateWrite) error {
	s.mu.Lock()
	defer s.mu.Unlock()
...
func TestAcceptanceUnauthorizedSubjectDegradesSilentlyWithoutLeaking(t *testing.T) {
	...
	if err := results.Save(context.Background(), priorPrincipal, priorResult, nil, nil, TimeAxisKeyFor(TimeContext{Axis: TemporalCurrent}), ReuseRetrievalIdentity{}, ReusePromptVersions{}, ReuseVersionAuthorities{}, 0, "", SemanticStateAbsent(SemanticStateAbsenceTurnEndedBeforeInterpretation)); err != nil {
