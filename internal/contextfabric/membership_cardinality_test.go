...
func TestFinalizingTwiceStatesOneCardinality(t *testing.T) {
	...
	for _, size := range []int{4, 0} {
		...
		t.Run(fmt.Sprintf("%d members", size), func(t *testing.T) {
			...
			once := engine.finalizeResult(context.Background(), storage.Principal{}, result, plan, frame, CanonicalFactBundle{})
			...
			twice := engine.finalizeResult(context.Background(), storage.Principal{}, once, plan, frame, CanonicalFactBundle{})
