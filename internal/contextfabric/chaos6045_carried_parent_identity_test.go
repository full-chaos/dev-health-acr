...
func TestStoredCarriedParentIdentityOverItsDecodeDomain(t *testing.T) {
	...
	cases := []struct {
		...
		{"state out of vocabulary, no identity", carriedStored(SemanticStateReadAvailable, encode(map[string]any{"state": "identity_guessed", "depth": 1})), true, true},
		{"state empty, no identity", carriedStored(SemanticStateReadAvailable, encode(map[string]any{"state": "", "depth": 1})), true, true},
		{"read malformed beside a decoded state", StoredInvestigationResult{SemanticStateRead: SemanticStateReadMalformed, SemanticState: &PersistedSemanticState{Extensions: SemanticStateExtensions{carriedParentIdentityExtension: valid}}}, false, false},
