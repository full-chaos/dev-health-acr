...
var ProductionColumns = map[string][]Column{
	...
	// work_graph_issue_pr is read by the shared work-item authorization
	// relation's pull-request-link path (dev-health-go
	// readers.WorkItemScopeSQL). Read live off the trial ClickHouse
	// (system.columns + SHOW CREATE TABLE, 2026-09-19): version_rank is
	// MATERIALIZED from provenance and last_synced and is the
	...
	"work_item_team_attributions": {
		...
		// (5) and source (7). Read by readTeamThemeMix's majority-vote
		// label (devhealthfacts/investment.go).
