...
func TestTheSiblingSweepIsExecuted(t *testing.T) {
	...
	owned := readPopulationEvidenceFrom(namedOperandFrame(SubjectTeam, SubjectTeam),
		...
		factsFor(alpha, kindList(flow, health)), MembershipCardinality{}, teamAssignment())
	...
	}, AnswerPlan{Requirements: []contractsv1.ContextFabricPlanRequirement{single}}, &frame, CanonicalFactBundle{}, pending, answerPassFirst, MembershipCardinality{})
