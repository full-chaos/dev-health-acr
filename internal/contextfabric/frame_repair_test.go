...
// discoveredTeamsEmphasisFrame is declared in frame_validation_test.go
// (shared with the shipped surface); this file uses that copy.
...
// operand at all.
// TestO12bSubjectOperandUnionFailuresAreRecordedAsI19 is declared in
// frame_validation_test.go (it stayed there deliberately at the carve: it
// tests invariant I19, not the bound). This file uses that copy.
...
// TestValidFrameEmitsTelemetryWithNoFailureThroughRepairWrapper is the
// "fires on EVERY frame reaching validation, INCLUDING valid ones" rule,
// exercised through ValidateAndRepairFrame (nil repairer) rather than
// plain ValidateFrame -- frame_validation_test.go's
// TestValidFrameEmitsTelemetryWithNoFailure covers the same property
// through the no-repair entry point; this covers the repair-aware one.
func TestValidFrameEmitsTelemetryWithNoFailureThroughRepairWrapper(t *testing.T) {
...
// TestWidenedObligationsAreAdvisoryAndNeverNarrow is declared in
// frame_validation_test.go (byte-identical here; it does not touch the
// repair path). This file uses that copy.
...
// TestFrameVersionIsAlwaysTheServerConstantThroughRepairWrapper is codex
// round 2's second finding, exercised through ValidateAndRepairFrame (nil
// repairer) rather than plain ValidateFrame --
// frame_validation_test.go's TestFrameVersionIsAlwaysTheServerConstant
// covers the same property through the no-repair entry point; this covers
// the repair-aware one.
//
...
// make.
func TestFrameVersionIsAlwaysTheServerConstantThroughRepairWrapper(t *testing.T) {
...
// TestGoalsAreCanonicalizedIntoASetThroughRepairWrapper is codex round 2's
// third finding, exercised through ValidateAndRepairFrame (nil repairer)
// rather than plain ValidateFrame -- frame_validation_test.go's
// TestGoalsAreCanonicalizedIntoASet covers the same property through the
// no-repair entry point (and additionally asserts the emitted EVENT, per
// the retro item on asserting the record and not only the object; that
// assertion is repeated here for the same reason).
//
...
func TestGoalsAreCanonicalizedIntoASetThroughRepairWrapper(t *testing.T) {
	proposed := QuestionFrame{
	...
	// AND THE EMITTED EVENT, not only the frame -- see the doc comment
	// above.
	event := FrameValidationEventFromRepair(proposed, result, "")
...
// TestEmphasisAndDimensionsAreCanonicalizedIntoSets is declared in
// frame_validation_test.go (byte-identical here; it exercises
// NormalizeFrame, which lives outside the repair path). This file uses
// that copy.
...
// TestOrganizationMemberKindIsValidatedWhenSupplied (F12, round 5's P2) is
// declared in frame_validation_test.go (byte-identical here; it exercises
// ValidateFramePhaseA1 directly, outside the repair path). This file uses
// that copy.
