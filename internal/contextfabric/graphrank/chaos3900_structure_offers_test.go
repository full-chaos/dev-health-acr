...
// TestAnchorOfferMaterial_AmbiguousSingleTermOffersItsOwnClaimants pins the
// CHAOS-4012 widening: a question with exactly ONE subject term whose
// identity-universe read named two-or-more claimants -- the corpus's
// dominant real-world anchor shape, and the one the pre-CHAOS-4012 builder
// structurally could never offer (anchorTermCandidates drops any term with
// != 1 claimants outright) -- must now surface EVERY one of that term's
// claimants as its own AnchorOption.
func TestAnchorOfferMaterial_AmbiguousSingleTermOffersItsOwnClaimants(t *testing.T) {
	t.Parallel()
	claimants := map[string][]IdentityMatch{
		"the api repo": {
			identityMatch(contractsv1.ContextFabricSubjectRepository, "org-a/api", "org-a/api"),
			identityMatch(contractsv1.ContextFabricSubjectRepository, "org-b/api", "org-b/api"),
		},
	}
	material := anchorOfferMaterial(claimants, true)
	if len(material.Missing) != 1 || material.Missing[0] != contractsv1.ContextFabricStructureNeedSubjectAnchor {
	...
	if len(material.AnchorOptions) != 2 {
		t.Fatalf("len(material.AnchorOptions) = %d, want 2 (both of the ambiguous term's claimants)", len(material.AnchorOptions))
	}
	ids := map[string]bool{}
	for _, opt := range material.AnchorOptions {
		ids[opt.CanonicalID] = true
		if opt.Kind != contractsv1.ContextFabricSubjectRepository {
			t.Errorf("AnchorOption.Kind = %q, want repository", opt.Kind)
		}
		if len(opt.MatchedTermHash) != 24 {
	...
	if !ids["org-a/api"] || !ids["org-b/api"] {
		t.Errorf("AnchorOptions ids = %v, want both org-a/api and org-b/api", ids)
	}
}

// TestAnchorOfferMaterial_MultiTermMultiClaimantMergesAcrossTerms pins the
// merge behavior when MULTIPLE terms are each independently ambiguous:
// every distinct (kind, canonical_id) claimant across every ambiguous term
// is offered, deduped once by (kind, canonical_id) even when two ambiguous
// terms share a claimant.
func TestAnchorOfferMaterial_MultiTermMultiClaimantMergesAcrossTerms(t *testing.T) {
	t.Parallel()
	claimants := map[string][]IdentityMatch{
		"the api repo": {
			identityMatch(contractsv1.ContextFabricSubjectRepository, "org-a/api", "org-a/api"),
		...
		"the deploy repo": {
			identityMatch(contractsv1.ContextFabricSubjectRepository, "org-a/deploy", "org-a/deploy"),
			// Shares one claimant with "the api repo" -- must be counted once.
			identityMatch(contractsv1.ContextFabricSubjectRepository, "org-a/api", "org-a/api"),
		},
	}
	material := anchorOfferMaterial(claimants, true)
	if len(material.AnchorOptions) != 3 {
		t.Fatalf("len(material.AnchorOptions) = %d, want 3 distinct (kind, canonical_id) candidates", len(material.AnchorOptions))
	}
	ids := map[string]bool{}
	for _, opt := range material.AnchorOptions {
		ids[opt.CanonicalID] = true
	}
	for _, want := range []string{"org-a/api", "org-b/api", "org-a/deploy"} {
		if !ids[want] {
			t.Errorf("AnchorOptions ids = %v, missing %q", ids, want)
		}
	}
}

// TestAnchorOfferMaterial_UniqueTermTakesPriorityOverAmbiguousClaimant pins
// the merge priority rule: when a UNIQUE-per-term candidate and an
// AMBIGUOUS term's claimant name the SAME (kind, canonical_id), the
// unique-term evidence (stronger: a term that resolves to exactly this
// entity and nothing else) is what the offer's label is derived from, not
// the ambiguous term's own weaker evidence. Two unique terms are used
// (not one) so BindAnchor's own decisiveness check (len(unique)==1) is
// NOT triggered -- this test is about merge priority within an already-
// ambiguous overall result, not the single-decisive-candidate short
// circuit TestAnchorOfferMaterial_UniqueClaimantOffersNothing covers.
func TestAnchorOfferMaterial_UniqueTermTakesPriorityOverAmbiguousClaimant(t *testing.T) {
	t.Parallel()
	claimants := map[string][]IdentityMatch{
		// Two unique terms naming two DIFFERENT anchors directly -- len(unique)==2,
		// so BindAnchor itself would still refuse (ReasonAnchorNotUnique).
		"org-a/api":   {identityMatch(contractsv1.ContextFabricSubjectRepository, "org-a/api", "org-a/api (canonical label)")},
		"org-c/other": {identityMatch(contractsv1.ContextFabricSubjectRepository, "org-c/other", "org-c/other")},
		// A SEPARATE ambiguous term whose own claimant set also includes org-a/api.
		"the api repo": {
			identityMatch(contractsv1.ContextFabricSubjectRepository, "org-a/api", "the api repo (ambiguous label)"),
			identityMatch(contractsv1.ContextFabricSubjectRepository, "org-b/api", "org-b/api"),
		},
	}
	material := anchorOfferMaterial(claimants, true)
	if len(material.AnchorOptions) != 3 {
		t.Fatalf("len(material.AnchorOptions) = %d, want 3 (org-a/api deduped once, org-c/other and org-b/api added)", len(material.AnchorOptions))
	}
	for _, opt := range material.AnchorOptions {
		if opt.CanonicalID == "org-a/api" && opt.Label != "org-a/api (canonical label)" {
			t.Errorf("org-a/api Label = %q, want the unique-term label to win over the ambiguous-term one", opt.Label)
		}
	}
}

// TestAnchorOfferMaterial_AmbiguousTermClaimantsAreCapped is the ambiguous
// widening's own companion to TestAnchorOfferMaterial_MoreThanMaxCandidatesIsCapped:
// a single term matching MORE than structureOfferMaxOptions distinct
// claimants must still cap deterministically, never mint an AnchorOptions
// list the wire Validate() would reject.
func TestAnchorOfferMaterial_AmbiguousTermClaimantsAreCapped(t *testing.T) {
	t.Parallel()
	...
	claimants := map[string][]IdentityMatch{"the repo": matches}
	material := anchorOfferMaterial(claimants, true)
	if len(material.AnchorOptions) != structureOfferMaxOptions {
		t.Fatalf("len(material.AnchorOptions) = %d, want %d (capped)", len(material.AnchorOptions), structureOfferMaxOptions)
	}
