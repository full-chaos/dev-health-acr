...
func TestF4_ResolveSubjectsReportsRetrievalDegradationOnTheResolution(t *testing.T) {
	...
	resolution, _, err := ResolveSubjects(context.Background(), storage.Principal{OrgID: "org_1"}, testRequest(),
		contextfabric.InterpretedQuestion{SubjectTerms: []string{"the auth work"}}, backend.deps(), nil, nil)
	if err != nil {
...
func TestF4_HealthySearchReportsNoDegradation(t *testing.T) {
	...
	resolution, _, err := ResolveSubjects(context.Background(), storage.Principal{OrgID: "org_1"}, testRequest(),
		contextfabric.InterpretedQuestion{SubjectTerms: []string{"anything"}}, backend.deps(), nil, nil)
	if err != nil {
		t.Fatalf("ResolveSubjects: %v", err)
	}
	if resolution.RetrievalDegraded {
