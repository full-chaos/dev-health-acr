...
func TestResolveSubjectsCommitsDocumentOnlyQuestionWithNoCanonicalParent(t *testing.T) {
	...
	resolution, _, err := ResolveSubjects(context.Background(), storage.Principal{OrgID: "org_1"}, testRequest(), testInterpreted("Platform Postmortem"), backend.deps())
	if err != nil {
...
func TestResolveSubjectsRetainsSharedParentAheadOfHigherScoringObservationsUnderTightBudget(t *testing.T) {
	...
	resolution, _, err := ResolveSubjects(context.Background(), storage.Principal{OrgID: "org_1"}, request, testInterpreted("Postmortem"), backend.deps())
	if err != nil {
		t.Fatalf("ResolveSubjects() error = %v", err)
	}
...
func TestResolveSubjectsCommitsReceiptSubjectOverHybridExactMatchUnderTightBudget(t *testing.T) {
	...
	resolution, _, err := ResolveSubjects(context.Background(), storage.Principal{OrgID: "org_1"}, request, testInterpreted("Project A"), backend.deps())
	if err != nil {
		t.Fatalf("ResolveSubjects() error = %v", err)
	}
...
func TestResolveSubjectsMarksCloseCandidatesAmbiguousAndOffersClarification(t *testing.T) {
	...
	resolution, _, err := ResolveSubjects(context.Background(), storage.Principal{OrgID: "org_1"}, testRequest(), testInterpreted("Which widget"), backend.deps())
	if err != nil {
		t.Fatalf("ResolveSubjects() error = %v", err)
	}
...
func TestResolveSubjectsClarificationPromptOnlyNamesRetainedCandidates(t *testing.T) {
	...
	request := testRequest()
	request.Options.MaxSubjectCandidates = 1
	resolution, _, err := ResolveSubjects(context.Background(), storage.Principal{OrgID: "org_1"}, request, testInterpreted("Which widget"), backend.deps())
	if err != nil {
		t.Fatalf("ResolveSubjects() error = %v", err)
	}
