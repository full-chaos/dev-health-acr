...
func TestDiscoveredCohortExcludesInternalBookkeepingSubjects(t *testing.T) {
	...
	cohort, _, _, _, _, _ := DiscoveredCohort(principal, discovery, []CandidateNode{impostorRoot, genuineTeam}, false, isReserved)
	if cohort == nil {
...
func TestDiscoveredCohortReportsAuthzDroppedCount(t *testing.T) {
	...
	discovery.Request.Options.MaxCohortMembers = 10
	cohort, authzDropped, kindScopedAuthzDropped, _, _, _ := DiscoveredCohort(principal, discovery, []CandidateNode{authorized, foreign}, false, noInternalSubjects)
...
func TestDiscoveredCohortKindScopedAuthzDroppedExcludesOtherKinds(t *testing.T) {
	...
	discovery.Request.Options.MaxCohortMembers = 10
	cohort, authzDropped, kindScopedAuthzDropped, _, _, _ := DiscoveredCohort(principal, discovery, []CandidateNode{deniedRepo, authorizedTeam}, false, noInternalSubjects)
	if cohort == nil || len(cohort.Members) != 1 || cohort.Members[0].Subject.CanonicalID != "team_platform" {
