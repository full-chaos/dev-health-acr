...
func TestResolveSubjects_RawSignalObserverReportsRawVectorSimilarity(t *testing.T) {
	...
	if _, _, err := ResolveSubjects(context.Background(), storage.Principal{OrgID: "org-1"}, request, interpreted, backend.deps(), nil, nil); err != nil {
...
func TestResolveSubjects_RawSignalObserverNilIsNoop(t *testing.T) {
	...
	resolution, _, err := ResolveSubjects(context.Background(), storage.Principal{OrgID: "org-1"}, request, interpreted, backend.deps(), nil, nil)
	if err != nil {
