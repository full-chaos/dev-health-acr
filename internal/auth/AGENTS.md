...
- Trusted web assertions use a separate permission vocabulary. `credential:issue` authorizes only a request-bound `POST /api/v1/oauth/device_approval`; that operation alone may carry the sole repository scope `["*"]`, while exact bounded device grants remain supported. Ordinary web reads remain exact/bounded and may never use the global wildcard. `credential:issue` is never a machine credential scope and never carries or returns an `fcacr_` secret. Device-record compare-and-set is the approval idempotency authority; the bounded per-process assertion replay cache remains request-replay mitigation.
