...
type KubernetesTokenReviewOptions struct {
	...
	APIServerURL string
	// ReviewerToken returns acr-api's OWN bearer token, authorized (via the
	// design brief's TokenReview RBAC: create on tokenreviews only) to
	// call TokenReview. Invoked once per Validate call so a kubelet-
	// rotated projected token is always current, matching how
	// CredentialSource is re-invoked per call in internal/sidecar.
	ReviewerToken func() (string, error)
	// Audience is the fixed ACR-specific audience TokenReview must confirm
	// the presented token was minted for (design brief:
	// "dev-health-acr-token-exchange"). Required.
	Audience string
	...
	// configures once, matching what acr.workload_bindings rows are
	// seeded against. Required.
	TrustDomain string
	Now         func() time.Time
}
