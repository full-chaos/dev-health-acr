...
Hardened local STDIO MCP client: HTTPS-only API boundary, credential precedence (env > keyring > file), bounded file reads, Git workspace discovery, and inert Markdown rendering. 81 files, 12.8k LOC, 49 test files. Flat package with logical prefixes: `api_client`, `config`, `credential`, `workspace`, `render`, `boundedfile`, `exec_resolver`.
...
| Credential resolution | `credential*.go` | Precedence: env > keyring > file; shape validation only |
...
- **Credential**: `credential_*_test.go` — precedence (env > keyring > file), shape validation, keyring timeout, file ownership (Unix).
