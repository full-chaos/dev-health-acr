...
- **Revoke before delete**: `CollectCredentialMaterial` enumerates every configured location and fails closed; callers revoke every distinct token and only then call `PurgeAllCredentialMaterial`. Never delete local material around a location that could not be read.
