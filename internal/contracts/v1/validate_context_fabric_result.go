...
func (r ContextFabricInvestigationResult) Validate() error {
	...
	if !stringLengthBetween(r.DirectJudgment, 0, 8000) || !stringLengthBetween(r.CurrentState, 0, 8000) || !stringLengthBetween(r.DeterministicAnswer, 1, 16000) || len(r.StrongestPressures) > 50 || !uniqueTrimmedStrings(r.StrongestPressures, 2000) || r.Drivers == nil || len(r.Drivers) > 50 || r.RemainingWork == nil || len(r.RemainingWork) > 250 || r.ReadinessGaps == nil || len(r.ReadinessGaps) > 250 || r.Paths == nil || len(r.Paths) > 250 || r.Conflicts == nil || len(r.Conflicts) > 250 || r.Limitations == nil || len(r.Limitations) > 250 || !uniqueTrimmedStrings(r.Limitations, 4000) || r.EvidenceRefIDs == nil || !boundedEvidenceRefs(r.EvidenceRefIDs, 500, true) || r.Warnings == nil || len(r.Warnings) > 250 || !uniqueTrimmedStrings(r.Warnings, 4000) {
