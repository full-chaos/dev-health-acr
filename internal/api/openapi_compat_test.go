...
func TestHostedReadRoutesMatchOpenAPI(t *testing.T) {
	...
	for _, route := range []struct{ path, method string }{
		{path: "/api/v1/context-fabric/model-config", method: "put"},
		{path: "/api/v1/context-fabric/model-config", method: "delete"},
	} {
		if hasSecurityAlternative(document.Paths[route.path][route.method].Security, "webAssertionAuth") {
			t.Fatalf("%s %s carries a credential and must remain bearer-only", route.method, route.path)
		}
	}
